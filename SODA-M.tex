...
\usepackage[left=1.18in,right=1.18in,top=0.98in,bottom=0.98in]{geometry}
...
\footnotetext[1]{Word Count: 2758}
...
\begin{abstract}
{\noindent\small
%We consider processes with feasibility constraints and metrics of interest, including process cost, that are stochastic functions of process controls. 
We propose an efficient one-stage stochastic optimization algorithm for the problem of finding process controls that minimize the expectation of cost while satisfying multiple deterministic and stochastic feasibility constraints with a given high probability. The proposed algorithm is based on a series of deterministic approximations 
to produce a candidate solution set
%of near-optimal control settings for the process, 
and on a refinement step using stochastic simulations  
%on the candidate set using 
with optimal simulation budget allocation. 
We conduct an experimental study for a real-world manufacturing service network, which shows that 
%In an experimental study, we demonstrate the proposed algorithm on a use case of a real-world heat-sink service network that involves contract suppliers and manufacturers as well as unit manufacturing processes of shearing, milling, drilling, and machining. The experimental study shows 
the proposed algorithm significantly outperforms four popular simulation-based stochastic optimization algorithms.\\ \\
...
%\vspace{2pt} % Some vertical space between the abstract and first section
\setstretch{2.0}
...
%
%Tools like SIMULINK \cite{Dabney:2001:MS:557989} and Modelica \cite{elmqvist1998modelica} allow users to run stochastic simulations on models of complex mechanical, hydraulic, thermal, control, and electrical power systems.
%Tools like OMOptim \cite{OMOptim}, Efficient Traceable Model-Based and Dynamic Optimization (EDOp) \cite{EDOp} use simulation models to heuristically-guide a trial and error search for an optimization solution.
...
%propose an integrated approach that combines simulation with MP where the MP problem is constructed from the original stochastic problem with uncertainties being resolved to their mean values by using a sample of black-box simulations. 
%This strategy of extracting an MP from the original problem is also used by 
...
%heuristic algorithm to solve a stochastic optimization problem of finding controls for temporal production processes with inventories to minimize the total cost while satisfying the stochastic demand with a predefined probability.
...
%To close this gap, we extended the heuristic algorithm from 
\cite{Krishnamoorthy2015} to 
%an algorithm called Stochastic Optimization Algorithm based on Deterministic Approximations (SODA) to solve the stochastic optimization problem over a 
...
%with service network that involves processes described using non-linear arithmetic \cite{GMU-CS-TR-2017-3}.
...
%to solve the problem of finding process controls that minimize the expectation of cost while satisfying multiple deterministic and stochastic feasibility constraints with a given high probability. 
...
%A SCFA simulation on input variable $\vec{X}$ is a sequence \\
%$y_1=expr_1,\\y_2=expr_2\\\vdots\\y_n=expr_n$
%\newline where $\forall_{1\le i \le n}~expr_i$
%is either\\
%\indent an arithmetic or boolean expression in terms of subset of elements from $\vec{X}$ or ${y_1,\dots,y_{i-1}}$, \\ \indent or\\
%\indent invokes $PD(\vec{P}$) 
...
%and a corresponding pseudo code is given in \algoSODAm.~
...
%In this way, g-SODA uses model knowledge from the white box simulation code in \textit{inflate} and \textit{deflate} phases, and optimal simulation budget allocation in the \textit{refineCandidates} phase to provide optimal control settings of the non-linear processes with multiple stochastic constraints that a process operator can use on the manufacturing floor in a stochastic environment. 
%The phases of g-SODA are explained in greater detail with the help of their pseudo-code in the subsections below.
...
%in terms of its quality of objective cost and rate of convergence as compared to four metaheuristic simulation-based optimization algorithms. 
...
%, or the finished components, i.e.,\ \textit{Accessory Package}.  
...
%Procuring the \textit{Heat Sink Base} presents an example of such a situation.  
...
%Another such example is the procurement of the \textit{Aluminum Plate}, wherein the OEM operates its own smelting plant or the aluminum bar stock is cut to specifications by another contract manufacturer. 
...
%Additionally, all production processes in HSPP were stochastic due to noise added to their controls. This noise has a mean of 0 and standard deviation between 1.8 to 2.8. 
...
%This is because in addition to the \textit{InflateDeflate} iterations, g-SODA is able to reduce the expected cost further in the \textit{RefineCandidates} phase with OCBA-CO simulation budget allocation. 
...
%This paper also demonstrates g-SODA on a use case of a real-world Heat Sink production and  conducts an experimental study that shows that g-SODA significantly outperforms the best competing algorithm by 48\% after one day and by 25\% after three days.
...
%(1) dynamically executing the inflate deflate phase and candidate refinement phase of g-SODA to improve the exploration of the search space,
%This dynamic execution also allows g-SODA to eventually converge towards the global optima as guaranteed by the competing algorithms considered in the experimental study. 
%(1) compare g-SODA's performance to methods and algorithms 
%including Bayesian optimization 
%that solve the same problem using techniques that typically extract the mathematical structure of the problem using black box simulations, and 
...
%(2) it would be interesting to research on whether evolutionary and other metaheuristic algorithms can be leveraged to improve the selection and refinement of the candidates collected in the \textit{InflateDeflate} phase.
