...
\title{\textbf{Stochastic Decision Optimization based on Deterministic Approximations of Processes described as Closed-form Arithmetic Simulation }}%\mycomment{to update} % Title
...
The experimental study demonstrates that g-SODA \mycomment{complete this after running experiments}
...
\indent\indent\indent$\theta_i,\dots,\theta_k \in \mathbb{R}^k$ are the constraint thresholds \\
...
For all the constraints where the probability of satisfaction of the stochastic constraint is not sufficiently high, a new inflated bound is computed as a function of number of standard deviations required to satisfy the constraint with desired probability. The assumption here is that the population is normally distributed and the standard deviation approaximately remains the same for the current bound and the inflated bound. Hence the inflated bound for the constraint is calculated by rearranging the terms in one-sample t-test (\algoInflate~lines 4-14). 
...
This returned inormation is then used in the \textit{PerformInflations} procedure to store the candidates and update the objective cost (\algoPerfInfl~lines 4-11). This inflation process continues with the new bounds calculated by the \textit{Inflate} procedure until the \textit{result} is \textit{accept} and inflation of the bound parameters are no longer necessary (line 12-16).
...
This returned inormation is then used in the \textit{PerformedDeflations} procedure to store the candidates and update the objective cost (\algoPerfDefl~lines 3-9). This deflation process continues with the new bounds calculated by the \textit{Deflate} procedure until the \textit{result} is \textit{reject} and deflation of bounds is no longer useful to  generate better candidates (line 12-13).
...
%TODO Write about application of this to DGS/DSS (Look at similar paragraph in paper 4)

%TODO Write about comparing this algo with baysian opt and other techniques/tools (look @ dicussion section of paper 10)
\section{Experimental Study}
\label{sec:expResults}

\begin{figure}
	\includegraphics[width=\textwidth]{images/HeatSink_contractSN.pdf}
	\caption{\mycomment{todo}}
	\label{fig:}       % Give a unique label
\end{figure}

\begin{figure}
	\includegraphics[width=\textwidth]{images/sodavsjmetal3day.png}
	\caption{\mycomment{todo}}
	\label{fig:results1days}       % Give a unique label
\end{figure}

\begin{figure}
	\includegraphics[width=\textwidth]{images/sodavsjmetal1day.png}
	\caption{\mycomment{todo}}
	\label{fig:results3days}       % Give a unique label
\end{figure}
%TODO 2 graphs: SODA for 1 day and 3 days with comparison 

%TODO Write about when SODA jumps down and whats going on there and why
